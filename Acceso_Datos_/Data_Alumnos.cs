...
using BackEnd.Modelos.EntityManual;
...
namespace BackEnd.Acceso_Datos
{
    ...
    public ResponseGeneric<List<Alumnos>> Buscar()
    {
        ...
        try
        {
                var responseDoomy = new List<Alumnos>() {
                    new Alumnos {idAlumno =1,nombre="Hector",apellidoMaterno="Rivera",apellidoPaterno="González" }

                };
                return new ResponseGeneric<List<Alumnos>>(responseDoomy);
                //return new ResponseGeneric<List<Alumnos>>(dbo.Alumnos.ToList());
        }
        catch (Exception ex)
        {
